...
Работает так чтобы добавленное перечёркивание выглядело так, будто Home Assistant умеет это сам:
...
* Влияет на уже отрисованную иконку и автоматически создаёт отсутствующее визуальное состояние — и только когда это действительно необходимо.
* Может добавить бледный круглый фон под иконкой, если штатного фона нет; 
* Никогда не меняет состояния сущностей, не вызывает сервисы, не вмешивается в автоматизации и не управляет устройствами.

Примечание: Внутренние интерфейсные компоненты Home Assistant не являются стабильным публичным API. Будущие обновления интерфейса могут потребовать обновления плагина.
