...
> Home Assistant позволяет использовать произвольные MDI-иконки, но не у каждой существует `-off` вариант. В итоге устройство уже выключено, а иконка визуально всё ещё выглядит активной. Этот плагин автоматически добавляет перечёркивание в стиле MDI для неактивных сущностей, когда штатной выключенной иконки нет.
...
Другие домены и состояния можно задать через необязательный глобальный файл конфигурации.
...
Если в конфигурации указан объект `states`, он полностью заменяет встроенный список. Плагин будет учитывать только перечисленные в нём домены и состояния.
...
В этом случае перечёркивание будет применяться только к `light` и `cover`. Встроенные правила для `switch`, `fan` и остальных доменов использоваться не будут.

Остальные параметры конфигурации независимы: если какой-либо из них не указан, для него сохраняется значение по умолчанию.

<a id="runtime-configuration"></a>
### Временная настройка через консоль

Метод `configure()` использует ту же логику.

Если передан `states`, он полностью заменяет текущий список:
