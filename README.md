...
## ⚙️ Какие состояния считаются выключенными

Если пользовательская конфигурация отсутствует, применяются следующие правила:
...
Конфигурация необязательна. Без неё плагин использует встроенный список выше.
...
Поскольку файл хранится на сервере Home Assistant, одни и те же правила применяются во всех браузерах и на всех устройствах.

Конфигурация хранится отдельно от файлов плагина, которыми управляет HACS, поэтому обновления плагина её не перезаписывают.

### Как применяется `states`

Если в конфигурации присутствует объект `states`, он **полностью заменяет** встроенный список. Плагин будет учитывать только явно перечисленные домены и состояния.
